...
from poliastro2.core.actions.propagate import (
    FarnocchiaPropagator,
    PropagatorKind,
)
from poliastro2.core.frames.util import get_frame
from poliastro2.core.orbit.creation import OrbitCreationMixin
from poliastro2.core.twobody.sampling import TrueAnomalyBounds
from poliastro2.math.events import elevation_function as elevation_function_fast
from poliastro2.math.threebody.soi import laplace_radius
from poliastro2.core.twobody.elements import eccentricity_vector, energy, t_p
from poliastro2.util import norm, wrap_angle
from poliastro2.warnings import PatchedConicsWarning
...
class Orbit(OrbitCreationMixin):
    """Represents the position and velocity of a body with respect to an attractor at a
    given epoch.

    This class provides methods to access various orbital elements and properties, such as position,
    velocity, semimajor axis, eccentricity, inclination, and more. It also includes methods to change
    the attractor, propagate the orbit, and apply maneuvers. The implicit reference system is an
    inertial one, typically the International Celestial Reference System (ICRS) for the Solar System.
    ...
    energy : ~astropy.units.Quantity
        Specific energy.
    e_vec : ~astropy.units.Quantity
        Eccentricity vector.
    h_vec : ~astropy.units.Quantity
        Specific angular momentum vector.
    h_mag : ~astropy.units.Quantity
        Specific angular momentum.
    arglat : ~astropy.units.Quantity
    ...
    def pqw(self):
        ...
        This method calculates the PQW vectors based on the eccentricity
        and inclination of the orbit. It issues a deprecation warning
        indicating that this method will be removed in a future release.
    ...
    def __str__(self):
        ...
        The format includes the radius of pericenter and apocenter, inclination,
        frame, attractor body, epoch, and time scale. The units for the radii
        are chosen based on the semimajor axis: astronomical units for large
        orbits and kilometers for smaller ones. If the frame is not implemented,
        a simplified format without the frame is used.
        """
        if self.a > 1e7 * u.km:
            unit = u.au
        else:
    ...
    def __repr__(self):
        """Return a string representation of the orbit."""
        return self.__str__()
    ...
    def to_ephem(self, strategy=TrueAnomalyBounds()):
        ...
        .. versionadded:: 0.17.0
        """
        from poliastro2.core.ephem import Ephem
    ...
    def plot(self, backend=None, label=None):
        ...
        from poliastro2.plotting.orbit.backends import Matplotlib2D, Plotly2D
