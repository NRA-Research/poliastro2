...
from poliastro2.core.frames.util import get_frame
from poliastro2.core.twobody.mean_elements import get_mean_elements
from poliastro2.core.twobody.states import (
    ClassicalState,
    ModifiedEquinoctialState,
    RVState,
)
from poliastro2.core.twobody.elements import (
    get_eccentricity_critical_argp,
    get_eccentricity_critical_inc,
    get_inclination_critical_argp,
    heliosynchronous,
)
from poliastro2.util import find_closest_value
from poliastro2.worldview.solar_sys.constants import J2000


class OrbitCreationMixin:
    """OrbitCreationMixin provides a collection of class methods to create orbital
    states from various representations such as position and velocity vectors, classical
    orbital elements, equinoctial elements, ephemerides, and more. It also includes
    utility methods to generate specific types of orbits like circular, stationary,
    synchronous, heliosynchronous, parabolic, and frozen orbits.

    Methods
    -------
    from_vectors(cls, attractor, r, v, epoch=J2000, plane=Planes.EARTH_EQUATOR)
        Creates an orbit from position and velocity vectors.
    from_coords(cls, attractor, coord, plane=Planes.EARTH_EQUATOR)
        Creates an orbit from an attractor and astropy SkyCoord or BaseCoordinateFrame instance.
    from_classical(cls, attractor, a, ecc, inc, raan, argp, nu, epoch=J2000, plane=Planes.EARTH_EQUATOR)
        Creates an orbit from classical orbital elements.
    from_equinoctial(cls, attractor, p, f, g, h, k, L, epoch=J2000, plane=Planes.EARTH_EQUATOR)
        Creates an orbit from modified equinoctial elements.
    from_ephem(cls, attractor, ephem, epoch)
        Creates an osculating orbit from ephemerides at a given epoch.
    from_sbdb(cls, name, **kwargs)
        Creates an orbit using SBDB data from Astroquery.
    circular(cls, attractor, alt, inc=0 * u.deg, raan=0 * u.deg, arglat=0 * u.deg, epoch=J2000, plane=Planes.EARTH_EQUATOR)
        Creates a circular orbit.
    stationary(cls, attractor)
        Creates a stationary orbit for the given attractor.
    synchronous(cls, attractor, period_mul=1 * u.one, ecc=0 * u.one, inc=0 * u.deg, argp=0 * u.deg, arglat=0 * u.deg, raan=0 * u.deg, epoch=J2000, plane=Planes.EARTH_EQUATOR)
        Creates a synchronous orbit where the orbital period matches the attractor's rotation rate.
    heliosynchronous(cls, attractor, a=None, ecc=None, inc=None, raan=0 * u.deg, argp=0 * u.deg, nu=0 * u.deg, epoch=J2000, plane=Planes.EARTH_EQUATOR)
        Creates a Sun-synchronous orbit using J2 perturbation.
    parabolic(cls, attractor, p, inc, raan, argp, nu, epoch=J2000, plane=Planes.EARTH_EQUATOR)
        Creates a parabolic orbit.
    frozen(cls, attractor, alt, inc=None, argp=None, raan=0 * u.deg, arglat=0 * u.deg, ecc=None, epoch=J2000, plane=Planes.EARTH_EQUATOR)
        Creates a frozen orbit by nullifying perturbations using critical inclinations and arguments of pericenter.
    ...
    def from_coords(cls, attractor, coord, plane=Planes.EARTH_EQUATOR):
        """Creates an `Orbit` from an attractor and astropy `SkyCoord` or
    ...
    def from_sbdb(cls, name, **kwargs):
        ...
        **kwargs
        ...
        """
        from poliastro2.core.math.io import orbit_from_sbdb
    ...
    def synchronous(
        ...
        r"""Returns an orbit where the orbital period equals the rotation rate of the
        orbited body.  The synchronous altitude for any central body can directly be
        obtained from Kepler's Third Law by setting the orbit period P\ :sub:`sync`,
        equal to the rotation period of the central body relative to the fixed stars D\
        :sup:`*`. In order to obtain this, it's important to match orbital period with
    ...
    def frozen(
        ...
        r"""Return a frozen Orbit. If any of the given arguments results in an
        impossibility, some values will be overwritten.
