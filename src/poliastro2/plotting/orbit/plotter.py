...
from poliastro2.core.twobody.sampling import EpochBounds
import poliastro2.plotting.orbit.backends as orbit_plotter_backends
from poliastro2.plotting.util import BODY_COLORS, generate_label
from poliastro2.util import norm, time_range
...
class Trajectory(
    ...
    Information contains all the trajectory coordinates, the current
    position, the name of the label to be displayed in the plotter, the
    color to be used and if dashed mode is desired or not.
    """
...
class OrbitPlotter:
    ...
    def set_body_frame(self, body, epoch=None):
        """Set perifocal frame based on the orbit of a body at a particular epoch if
        given.
