...
def distance(cartesian_cords, px, py, pz):
    """Calculates the distance from an arbitrary point to the given location (Cartesian
    coordinates).
...
def is_visible(cartesian_cords, px, py, pz, N):
    """Determine whether an object located at a given point is visible from the given
    location.
...
def cartesian_to_ellipsoidal(a, c, x, y, z):
    """Converts cartesian coordinates to ellipsoidal coordinates for the given
    ellipsoid. Instead of the iterative formula, the function uses the approximation
    introduced in Bowring, B. R. (1976). TRANSFORMATION FROM SPATIAL TO GEOGRAPHICAL
    COORDINATES.
